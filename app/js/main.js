$(function(){
    ...
    $('.header__theme').on('click', function(){
        $('body, .header__burger, .header__burger-dark, .header__top-title-img, .header__top-title-img-dark, .absolute, .header__logo, .header__logo-dark, .header__feedback, .header__btn, .header__bottom-text, .benefits__line, .brands__item, .title, subtitle, .feedback__text, .phrase, .feedback__form-item, .location__address-btn-text, label[for="request-btn"], .swiper-button-next, .swiper-button-prev').toggleClass('dark--theme');
